...
const BRENNA = preload("res://characters/brenna/BrennaAndLuke-2.png (8).png")
const LUKE = preload("res://characters/luke (player)/lukeTransparent.png")
	...
	match MainInfo.leader:
		"Luke":
			...
			for follow in followers:
				follow.sprite.texture = BRENNA
		"Brenna":
			#tag_photo.texture = Resources.BRENNA_TAG_LUKE
			sprite.texture = BRENNA
			for follow in followers:
				follow.sprite.texture = LUKE
...
func move_false():
	...
	if queue_menu:
		...
		stopped_moving.emit()
		if get_tree().current_scene.has_node("LoopWorld"):
			get_tree().current_scene.get_node("LoopWorld").checkPos(input_dir)
